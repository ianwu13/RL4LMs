...
"""
IAN EXPERIMENT CLASSES
NOTE: Some classes may be unecessary, check later
"""
...
Sel: selection token output
"""
class CaSiNoSupGenSelOracle(Dataset):
    ...
    def get_input_seq(self, agent_context, dialogue, opponent_cont=''):
        """Construct the input seq from agent_context and dialogue."""
        ...
        input_seq = f"{agent_context} {opponent_cont} <history> {dial2}".strip()
    ...
    def process_each_split(self):
        ...
        for dialogue in tqdm(all_dialogues):
            ...
            for c in dialogue['chat_logs']:
                ...
                id = c['id']
                if id == a1:
                    inp_seq = self.get_input_seq(agent_1_context, dialogue_1[:], opponent_cont=agent_2_context)
                    processed_data.append(f'"{inp_seq}","<you> {sentence}","{agent_2_context}"\n')
                    dialogue_1.append(f'<you> {sentence}')
                    dialogue_2.append(f'<them> {sentence}')
                elif id == a2:
                    inp_seq = self.get_input_seq(agent_2_context, dialogue_2[:], opponent_cont=agent_1_context)
                    processed_data.append(f'"{inp_seq}","<you> {sentence}","{agent_1_context}"\n')
                    dialogue_1.append(f'<them> {sentence}')
                    dialogue_2.append(f'<you> {sentence}')
                else:
...
Sel: selection token output
"""
class DealornodealSupGenSelOracle(Dataset):
...
Sel: selection token output
"""
class CaSiNoSupGenSelNoOPTPref(Dataset):
...
Sel: selection token output
"""
class DealornodealSupGenSelNoOPTPref(Dataset):
